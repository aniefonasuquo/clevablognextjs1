  ...
  return (
    <>
        {allQuestions.map(question => (
          <div className={display == allQuestions.indexOf(question)? `${styles.visible}` : `${styles.question}`}>
          <div className={styles.radio} id={question.name}>
            <div>{question.title}</div>
            <div>  
            {question.options.map(option => (
              <label htmlFor={option.name}>
                  <input type={question.type} name={question.name} value={option.value} id={option.name}/><p>{option.name}</p>
                </label>
            ))}     
            </div>
          </div>
    ...
    <div className={styles.pagecontainer}>
      ...
      <div className={styles.formcontainer}>
        ...
        <div className={styles.controls}>
            {display == 0 && (<button type='button' className={styles.prevbutton} onClick={enterTest}>Take Quiz</button>)}
            {display > 0 && (<button type='button' className={styles.prevbutton} onClick={prevquestion}>previous</button>)}
