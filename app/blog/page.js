...
export default async function Homepage() {
  ...
  <section className={styles.topDiv}>  
    ...
    <div className={styles.topright}> 
      <h1 className={`${monteserrat.className} ${styles.recentPost}`}><Link href='/blog/archives' >Recent Posts</Link></h1>
      <div className={styles.recentpostswrapper}>
          ...
          <div className={styles.rArticle} key={id}>
            ...
            <div className={styles.rArticleDetails}>           
              <Link href={`/blog/category/${tags[0]}`}><span>{tags[0]}</span></Link>
              <Link href={`/blog/posts/${slug}`}><h2 className={monteserrat.className}>{title}</h2></Link>
            </div>
          </div>
          ))}
      </div>
      ...
      {posts.slice(0,3).map(({id, tags, title, slug}) => (
        <div className={styles.blotterPost} key={id}>
          <div className={styles.blotterPostImg}>
            <Link href={`/blog/posts/${slug}`}><Image sizes="100vw" className={styles.catimage} fill='true' src={img} alt=""/> </Link>
          </div>
          <div>
            <Link className={`${monteserrat.className}`} href={`/blog/category/${tags[0]}`}><span className={styles.blottercategory}>{tags[0]}</span></Link>     
            <Link href={`/blog/posts/${slug}`}> <h2 className={`${monteserrat.className} ${styles.blottertitle}`}>{title}</h2></Link>
          </div>
        </div>
      ))}
    </div>
